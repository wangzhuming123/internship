<template>
  <div class="app-container">
    ...
    <el-form :inline="true">
      ...
      <template>
        <el-select v-model="searchObj.classId" placeholder="请选择">
          <el-option
            v-for="item in List"
            :key="item.classId"
    ...
    <el-table id="excel_table" :data="list" border stripe style="width: 100%">
      ...
      <el-table-column label="结束时间" width="120px" align="center">
        <template slot-scope="scope">
          {{
            scope.row.interEndTime
              ? scope.row.interEndTime.substr(0, 10)
              : "暂无"
          }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      internshipApi.getEdByTeaId(teaId, this.searchObj).then((response) => {
